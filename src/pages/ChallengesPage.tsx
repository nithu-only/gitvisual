...
const challenges: Challenge[] = [
  { title: 'Create Your First Branch', description: 'Initialize a repository and create a branch called "develop".', startingCommands: ['git init'], solution: ['git branch develop'], validate: (state) => state.branches.some((b: any) => b.name === 'develop'), hint: 'Use: git branch <name>' },
  { title: 'Fast-Forward Merge', description: 'Create a feature branch, add a commit, then fast-forward merge it into main.', startingCommands: ['git init', 'git add .', 'git commit -m "Initial commit"', 'git branch feature', 'git switch feature', 'git add feature.txt', 'git commit -m "Add feature"'], solution: ['git switch main', 'git merge feature'], validate: (state) => { const main = state.branches.find((b: any) => b.name === 'main'); const feature = state.branches.find((b: any) => b.name === 'feature'); return main && feature && main.commitId === feature.commitId; }, hint: 'Switch to main first, then merge feature.' },
  { title: 'Three-Way Merge', description: 'Create a diverged history and perform a merge that creates a merge commit.', startingCommands: ['git init', 'git add .', 'git commit -m "Initial commit"', 'git branch feature', 'git switch feature', 'git add feature.txt', 'git commit -m "Feature work"', 'git switch main', 'git add main.txt', 'git commit -m "Main work"'], solution: ['git merge feature'], validate: (state) => { const main = state.branches.find((b: any) => b.name === 'main'); if (!main) return false; const commit = state.commits[main.commitId]; return commit && commit.parentIds.length === 2; }, hint: 'Switch to main, then merge feature.' },
  { title: 'Rebase Feature onto Main', description: 'Rebase a feature branch onto main to linearize the history.', startingCommands: ['git init', 'git add .', 'git commit -m "Initial commit"', 'git branch feature', 'git switch feature', 'git add feature.txt', 'git commit -m "Feature work"', 'git switch main', 'git add main.txt', 'git commit -m "Main work"', 'git switch feature'], solution: ['git rebase main'], validate: (state) => { const feature = state.branches.find((b: any) => b.name === 'feature'); if (!feature) return false; const commit = state.commits[feature.commitId]; return commit && commit.parentIds.length === 1; }, hint: 'Make sure you are on the feature branch.' },
  { title: 'Soft Reset', description: 'Undo the last commit but keep changes staged.', startingCommands: ['git init', 'git add .', 'git commit -m "First commit"', 'git add file.txt', 'git commit -m "Second commit"'], solution: ['git reset --soft HEAD~1'], validate: (state) => { const main = state.branches.find((b: any) => b.name === 'main'); if (!main) return false; const commit = state.commits[main.commitId]; return commit && commit.message === 'First commit' && Object.keys(state.stagingArea).length > 0; }, hint: 'Use: git reset --soft HEAD~1' },
  { title: 'Revert a Commit', description: 'Revert the last commit by creating an undo commit.', startingCommands: ['git init', 'git add .', 'git commit -m "Initial commit"', 'git add bad-file.txt', 'git commit -m "Bad commit"'], solution: ['git revert HEAD'], validate: (state) => { const main = state.branches.find((b: any) => b.name === 'main'); if (!main) return false; const commit = state.commits[main.commitId]; return commit && commit.message.includes('Revert'); }, hint: 'Use: git revert HEAD' },
];
